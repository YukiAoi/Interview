...
1.  var定义的变量，`没有作用域的概念`。\
    let定义的变量，只能在作用域里访问，不能跨作用域访问，也不能跨函数访问。\
...
4.  在全局上下文中，基于let声明的全局变量和全局对象GO（window）没有任何关系 ;\
    var声明的变量会和GO有映射关系；
5.  `let`和`const`会产生`暂时性死区`（因为有作用域，在被声明之前都不能使用）
...
apply和call传入的参数列表形式不同。apply 接收 arguments （伪数组），call接收一串参数列表
```
fn.call(obj, 1, 2);
fn.apply(obj, [1, 2]);
```
...
总结：基于Function.prototype上的 `apply、call和bind`调用模式，这三个方法都可以显示的指定调用函数的 this 指向。`apply`接收参数的是数组，`call`接受参数列表，`bind`方法通过传入一个对象，返回一个`this` 绑定了传入对象的新函数。这个函数的  `this`指向除了使用`new`时会被改变，其他情况下都不会改变。若为空默认是指向全局对象window。
...
4.  `箭头函数不绑定 Arguments 对象`。取而代之用rest参数...解决。由于 箭头函数没有自己的this指针，通过 call() 或 apply() 方法调用一个函数时，只能传递参数（不能绑定this），他们的第一个参数会被忽略。（这种现象对于bind方法同样成立）
